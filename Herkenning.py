...
while True:
      ...
      for i, plate in enumerate(results['results']):
          ...
          if (best_candidate['confidence'] > 93):
               ...
               if (platevalues.count(best_candidate['plate']) == 3):
          #if (best_candidate['confidence']) > 93:
                   print ('Plate #{}: {:7s} ({:.2f}%)'.format(i, best_candidate['plate'].upper(), best_candidate['confidence']))
                   #print(best_candidate['plate'].upper())
                   plaat = best_candidate['plate'].upper()
                   f.write(best_candidate['plate'].upper()+ '\n')
              #f.write('Plate #{}: {:7s} ({:.2f}%)'.format(i, best_candidate['plate'].upper(), best_candidate['confidence']))
                   #f.write("\n")
                   #vanaf hier terug wegdoen moest het er niet bij moeten
